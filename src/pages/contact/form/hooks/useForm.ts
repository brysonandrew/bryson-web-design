...
import { TChangeEvent, TFocusEvent, TFormKey, TInputHandlers, TStatus } from '../../config';
...
export const useForm = ({ element }: TConfig) => {
  ...
  const handleBlur = (
    ...
    const target = event.currentTarget;
    if (!target) return;
    if (target.name === currentRef.current.contact.focusKey) {
      updateFocus(null);
    }
  };
  ...
  const handleChange = ({
    ...
    dispatch({ type: "contact-form", value: { [name]: value, } });
  };
